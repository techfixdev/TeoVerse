...
Defines the generic e-Sword `.bblx` parser and its consumers for importing Bible versions from e-Sword SQLite files into TeoVerse. The parser is version-agnostic; version-specific data lives in separate manifests.
...
The generic parser module MUST NOT contain any version-specific logic, book names, or hardcoded paths. All version-specific data MUST be provided by the caller via the manifest.
...
- THEN no version-specific paths, names, or identifiers are found
