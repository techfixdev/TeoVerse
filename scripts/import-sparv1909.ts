...
const zipPath = path.join(sourceDir, `${SPARV1909_SOURCE.slug}_usfm.zip`);

export async function importSparv1909() {
  const zip = await readOrDownloadZip();
  const entries = unzipSync(new Uint8Array(zip));

  // Upsert the spaRV1909 Bible recurso
  ...
  const recurso =
    createdRecurso ??
    (await db.select().from(recursos).where(eq(recursos.slug, SPARV1909_SOURCE.slug)).get());
  if (!recurso) throw new Error(`Could not create or load recurso ${SPARV1909_SOURCE.slug}.`);
